  ...
  methods: {
    async blockUser() {
      const response = await fetch(
        `http://${process.env.VUE_APP_BACK}/users/block/` +
    ...
    async unBlockUser() {
      const response = await fetch(
        `http://${process.env.VUE_APP_BACK}/users/unblock/` +
          store.getters.GET_USER.id +
          "/" +
          store.getters.GET_USER_TARGET.id,
        {
    ...
    async desactivedQrCode() {
      await fetch(`http://${process.env.VUE_APP_BACK}/2fa`, {
        method: "DELETE",
        mode: "cors",
        credentials: "include",
        headers: {
    ...
    async get_user() {
      var response = await fetch(
        `http://${process.env.VUE_APP_BACK}/users/` + this.save_username,
        {
          method: "GET",
          mode: "cors",
          credentials: "include",
    ...
    async sendAvatar() {
      ...
      fetch(`http://${process.env.VUE_APP_BACK}/avatar`, {
        method: "PATCH",
        mode: "cors",
        credentials: "include",
        headers: {
    ...
    async submit() {
      let response = await fetch(`http://${process.env.VUE_APP_BACK}/users/update`, {
        method: "PATCH",
        mode: "cors",
        credentials: "include",
        headers: {
