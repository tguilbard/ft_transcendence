...
export default class Chat extends Vue {
	...
	private async getMode(user_target: string) {
		const response = await fetch(`/channel/mode/` + store.getters.GET_CHAN_CURRENT.realname + "/" + user_target, {
			method: "GET",
			mode: "cors",
			credentials: "include",
			headers: {
	...
	private async getMyMode() {
		...
		if (!chanCurrent || !chanCurrent.realname)
			return;
		const response = await fetch(`/channel/mode/` + store.getters.GET_CHAN_CURRENT.realname + "/" + store.getters.GET_USER.username, {
			method: "GET",
			mode: "cors",
			credentials: "include",
			headers: {
	...
	private async getListChannelPublic() {
		const response = await fetch(`/channel/ListChannelPublic`, {
			method: "GET",
			mode: "cors",
			credentials: "include",
			headers: {
	...
	private async getMessagesInChannel(chanName: string) {
		const response = await fetch(`/channel/MessagesInChannel/` + chanName, {
			method: "GET",
			mode: "cors",
			credentials: "include",
			headers: {
	...
	private async getChanListByMode(chanMode: string): Promise<ChannelEntity[]> {
		const response = await fetch(`/getChanListByMode/` + chanMode, {
			method: "GET",
			mode: "cors",
			credentials: "include",
			headers: {
